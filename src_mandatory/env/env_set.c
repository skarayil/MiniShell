...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/12 15:03:33 by skarayil          #+#    #+#             */
...
static char	*mk_entry(const char *k, const char *v)
{
	size_t	kl;
	size_t	vl;
	char	*s;
...
static int	env_append(t_shell *sh, char *entry)
{
	int		n;
	char	**newp;
	int		i;
...
int	env_set(t_shell *sh, const char *key, const char *val)
{
	...
	if (!sh || !key)
		return (1);
	i = env_find_key(sh->envp, key);
	entry = mk_entry(key, val);
	if (!entry)
		return (1);
	if (i >= 0)
	{
