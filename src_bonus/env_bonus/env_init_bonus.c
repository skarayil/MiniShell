...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/12 15:19:07 by skarayil          #+#    #+#             */
...
static int	fill_minimal_env_bonus(char **env, char *pwd)
{
	...
	if (!env[2])
		return (1);
	env[3] = ft_strdup("_=./minishell_bonus");
	if (!env[3])
		return (1);
	env[4] = NULL;
	return (0);
...
char	**env_init_minimal_bonus(char **envp)
{
	...
	cwd = getcwd(NULL, 0);
	if (!cwd)
		return (env_free_bonus(env), NULL);
	pwd = ft_strjoin("PWD=", cwd);
	free(cwd);
	if (!pwd)
		return (env_free_bonus(env), NULL);
	if (fill_minimal_env_bonus(env, pwd))
		return (env_free_bonus(env), NULL);
	return (env);
}
...
char	**env_copy_bonus(char **envp)
{
	...
	while (envp[i])
	{
		...
		if (!copy[i])
			return (env_free_bonus(copy), NULL);
		i++;
	}
	copy[i] = NULL;
	return (copy);
