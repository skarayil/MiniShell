...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/12 15:16:28 by skarayil          #+#    #+#             */
...
static t_fd_node	*create_fd_node_bonus(int fd, const char *tmp_file)
{
	t_fd_node	*node;
...
void	ms_gc_add_fd_bonus(t_shell *sh, int fd, const char *tmp_file)
{
	...
	if (fd < 0 || !sh)
		return ;
	node = create_fd_node_bonus(fd, tmp_file);
	if (!node)
		return ;
